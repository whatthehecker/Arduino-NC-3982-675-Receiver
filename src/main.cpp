...
const size_t MAX_SIGNAL_COMPONENTS = 256;

size_t currentSignalIndex = 0;
int32_t signalComponents[MAX_SIGNAL_COMPONENTS];

uint32_t currentCounter = 0;

// Resolution in microseconds, any signal with a duration smaller than this might not get detected.
// Measurements of durations will be multiples of this value.
const int RESOLUTION_USECS = 10;
...
void IRAM_ATTR onReceiverChanged()
{
  ...
  if (currentSignalIndex >= MAX_SIGNAL_COMPONENTS)
  {
    return;
  }

  bool isNowHigh = digitalRead(RECEIVER_PIN) == HIGH;
  long timeDiff = micros() - lastTime;
  // Write gaps as negative values (if now high, must have been a gap before)
  // and pulses as positive values (if now low, must have been the start of a pulse before).
  int32_t valueToWrite = (isNowHigh ? -1 : 1) * timeDiff;

  signalComponents[currentSignalIndex] = valueToWrite;
  currentSignalIndex++;
  currentSignalIndex = (currentSignalIndex + 1) % MAX_SIGNAL_COMPONENTS;
...
void printDecodedData(uint8_t *data) {
  ...
  uint8_t humidityRaw = (data[3] & 0x0F) * 10 + (data[4] >> 4);
  uint8_t channelRaw = data[4] & 0x0F;
  uint8_t isButtonPress = (data[1] >> 3) & 1;

  float tempFahrenheit = (temperatureRaw - 900) * 0.1f;
  float tempCelsius = (tempFahrenheit - 32.0) / 1.8;

  char buf[256];
  snprintf(buf, sizeof(buf), "ID: %02x\nTemp Fahrenheit: %2.2f\nTemp Celsius: %2.2f\nHumidity: %u %%\nChannel: %d\nIs button press: %d", id, tempFahrenheit, tempCelsius, humidityRaw, channelRaw, isButtonPress);
  Serial.println(buf);
...
void loop()
{
  if (currentSignalIndex > 0)
  {
    ...
    {
      int32_t currentDuration = signalComponents[i];

      // Ignore pulses, we are searching for gaps.
      if (currentDuration >= 0)
      {
        continue;
      }

      if(!didFindPrefix && isBetween(abs(currentDuration), SYNC_PREFIX_GAP_DURATION - TOLERANCE, SYNC_PREFIX_GAP_DURATION + TOLERANCE)) {
        didFindPrefix = true;
      ...
      if(didFindPrefix) {
        currentDuration = abs(currentDuration);

        if(isBetween(currentDuration, SYNC_POSTFIX_GAP_DURATION - TOLERANCE, SYNC_POSTFIX_GAP_DURATION + TOLERANCE)) {
        ...
        bool isHigh = isBetween(currentDuration, HIGH_GAP_DURATION - TOLERANCE, HIGH_GAP_DURATION + TOLERANCE);
        bool isLow = isBetween(currentDuration, LOW_GAP_DURATION - TOLERANCE, LOW_GAP_DURATION + TOLERANCE);

        if(!isLow && !isHigh) {
          char buf[80];
          snprintf(buf, sizeof(buf), "Found invalid gap with length %ld, going back to prefix search.", signalComponents[i]);
          Serial.println(buf);
    ...
    memset(signalComponents, 0, MAX_SIGNAL_COMPONENTS);
  }
}